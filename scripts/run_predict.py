...
parser.add_argument("--is_training", type=bool, default=False, help="whether to train bert model")
...
parser.add_argument("--predict_batch_size", type=int, default=4, help="Total batch size for training.")
...
parser.add_argument("--predict_column_names", type=str, help="Colunms name used to predict, separated by ','. E.g., 'col1, col2,'")
...
def predict():
  ...
  #bert_config_file = os.path.join(args.bert_model, "bert_config.json")
  ...
  predict_examples = processor.get_test_examples(df, test_column_names, args.label_num)
  ...
  print(out_df.head(3))
  out_df.to_csv(output_predict_file)
